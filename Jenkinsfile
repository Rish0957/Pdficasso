            ...
            steps {
                script {
                    if (env.GIT_BRANCH == 'origin/dev') {
                        ...
                        sh 'FRONTEND_PORT=80 BACKEND_PORT=3000 VITE_API_URL=http://localhost:3000 COMPOSE_PROJECT_NAME=pdficasso-prod docker compose up -d --build'
                        
                    } else {
                        echo "🚀 Deploying to DEV Environment (Port 8082) for branch ${env.GIT_BRANCH}..."
                        sh 'FRONTEND_PORT=8082 BACKEND_PORT=3003 VITE_API_URL=http://localhost:3003 COMPOSE_PROJECT_NAME=pdficasso-dev docker compose up -d --build'
                    }
                }
            }
        }
